...
const Navigation = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled 
        ? 'bg-white/95 backdrop-blur-sm shadow-soft border-b border-ocean-light/20' 
        : 'bg-transparent'
    }`}>
      <div className="container mx-auto px-6">
            ...
            <div className="ml-10 flex items-baseline space-x-4">
              {navItems.map((item) => (
                <button
                  ...
                  className={`px-3 py-2 rounded-md text-sm font-medium transition-colors hover:text-ocean-primary ${
                    isScrolled 
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 bg-white/95 backdrop-blur-sm rounded-lg mt-2 shadow-soft">
              {navItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
